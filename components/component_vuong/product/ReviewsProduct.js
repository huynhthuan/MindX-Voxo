   ...
   return (
      <div className="tab-pane fade" id="review">
         <div className="row g-4">
            <CustomReviews listReviews={listReviews} average_rating={average_rating} rating_count={rating_count} />

            <ReviewPost id={id} listReviews={listReviews} setListReviews={setListReviews} />

            <div className="col-12 mt-4">
               <div className="customer-review-box">
                  <h4>Customer Reviews</h4>
                  {!data.data[0] && <p className="alert alert-warning mt-3 w-100 text-center">There are no review in this product</p>}
                  <LoadMore totalItems={totalItems} id={id}>
                     {listReviews.map(({ date_created, reviewer_avatar_urls, review, rating, reviewer, ...props }, index) => (
                        <div key={index} className="customer-section ">
                           <div className="customer-profile">
                              <img src={reviewer_avatar_urls["96"]} className="img-fluid blur-up lazyload" alt="" />
                           </div>
                           <div className="customer-details">
                              <h5>{reviewer}</h5>
                              <RatingDetails average_rating={rating} />
                              <p className="font-light" dangerouslySetInnerHTML={{ __html: review }}></p>
                              <p className="date-custo font-light">- {moment(date_created).format("LL")}</p>
                           </div>
                        </div>
                     ))}
                  </LoadMore>
